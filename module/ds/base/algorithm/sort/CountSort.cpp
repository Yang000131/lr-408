//
// Created by 一可爱小白兔 on 2025-08-28 10:24.
//
 ...
 * 计数排序
 *
 * <li>算法思想：将待排序的数组中的元素值作为计数数组的索引，其个数作为计数数组的元素值，遍历辅助数组排序
 * <li>空间换时间：时间复杂度：O(n + k) 空间复杂度：O(k)
 * <li>稳定性：非比较的稳定排序
 * <li>适用性：适用于数据范围不大的整数排序问题，尤其是当输入数据集中且分布均匀时效果最佳
 */
class Solution_One {
public:
 ...
 *
 * <ol type="i">
 * <li>找出待排序数组中的最大值和最小值，确定计数数组的大小
 * <li>创建一个计数数组，统计每个元素出现的次数
 * <li>对计数数组进行累加，得到每个元素在排序后数组中的位置
 * <li>创建一个临时数组，从后向前扫描原数组，根据计数数组确定元素位置
 * <li>将临时数组复制回原数组
 */
class Solution_Two {
public:
